...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run PFNet inference on HDX-MS data with comprehensive output generation"
    )
    ...
    parser.add_argument(
        "--model_type", type=str, choices=["envelope", "centroid"], default="envelope", help="Model type"
    )
    ...
    parser.add_argument(
        "--refine_single_pos_conf_threshold",
        type=float,
        default=0.8,
        help="Single position confidence threshold for refinement",
    )
    parser.add_argument(
        "--refine_non_single_pos_conf_threshold",
        type=float,
        default=0.9,
        help="Non-single position confidence threshold for refinement",
    )
    ...
    parser.add_argument(
        "--generate_all", action="store_true", help="Generate all outputs (equivalent to enabling all generate_* flags)"
    )
    ...
    parser.add_argument(
        "--generate_bfactor_plot", action="store_true", default=True, help="Generate BFactor plot for PDB"
    )
...
def main():
    ...
    analysis_objs = {
        f"{state_names[idx]}_{idx}": output_dicts[f"{state_names[idx]}_{idx}"]["analysis_objs"]["analysis_pfnet"]
        for idx in range(len(state_names))
        if f"{state_names[idx]}_{idx}" in output_dicts
    }
