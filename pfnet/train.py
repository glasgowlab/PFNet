...
if __name__ == "__main__":
    ...
    trainer = L.Trainer(
        ...
        callbacks=[
            ...
            ModelCheckpoint(save_weights_only=False, filename="{epoch}-{val_loss:.0f}", monitor="val_loss", mode="min"),
        ],
        logger=wandb_logger,
    ...
    for i in range(config["data_start_idx"], config["data_end_idx"], chunk_size):
        train_dataset = OnlineHDXDataset(
            data_start_idx=i, data_end_idx=i + chunk_size, base_seed=42, generate_miss_id=True
        )
