...
class FeatherPyroModel(PyroModule):
    ...
    def model(self, batch):
        ...
        with pyro.plate("timepoints_centroid"):
            ...
            pyro.sample(
                "centroid",
                dist.Normal(loc=pred_centroid, scale=self.cen_sigma.expand(pred_centroid.shape)),
                obs=obs_centroid,
            )
    ...
    def _hx_forward(self, batch, log_kex):
        ...
        pred_envelope = get_calculated_isotope_envelope(
            ...
            obs_envelope_t0,
            obs_envelope,
            log_kex.unsqueeze(0).to(self.device),
            back_ex,
            saturation,
        )
    ...
    def _hx_cen_forward(self, batch, log_kex):
        ...
        # mask = self.get_mask()
        pred_num_d = get_calculated_num_d(
            start_pos, end_pos, time_points, log_kex.unsqueeze(0).to(self.device), back_ex, saturation
        )
...
class FeatherMHKernel(MCMCKernel):
    ...
    def log_kex_to_grid(self, log_kex):
        ...
        indices = torch.clamp(
            torch.round((log_kex - self.grid[0]) / (self.grid[-1] - self.grid[0]) * (self.grid_size - 1)),
            0,
            self.grid_size - 1,
        )
    ...
    def _compute_metropolis_acceptance(self, proposed_potential_val, current_potential_val, temperature=None):
        ...
        log_accept_ratio = torch.tensor(
            -(proposed_potential_val - current_potential_val) / temperature, device=self.device
    ...
    def sample(self, params):
        ...
        if self.swap_stage:
            for i in range(num_moves):
                ...
                accept_prob = self._compute_metropolis_acceptance(
                    proposed_potential_val,
                    current_potential_val,
                    temperature=torch.tensor(1e-10, device=self.device),
        ...
        for i in range(num_moves):
            ...
            # Evaluate proposed state
            proposed_params = {
                "log_kex": self.grid_to_log_kex(proposed_grid_idx),
                **{k: v for k, v in params.items() if k != "log_kex"},
            }
